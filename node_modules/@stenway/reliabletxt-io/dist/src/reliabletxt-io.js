...
export class ReliableTxtFile {
    ...
    static appendAllTextSync(content, filePath, createWithEncoding = ReliableTxtEncoding.Utf8, prependLineBreakIfNotEmpty = false) {
        this.appendSync(content, filePath, createWithEncoding, prependLineBreakIfNotEmpty);
    }
    static async appendAllText(content, filePath, createWithEncoding = ReliableTxtEncoding.Utf8, prependLineBreakIfNotEmpty = false) {
        await this.append(content, filePath, createWithEncoding, prependLineBreakIfNotEmpty);
    }
...
export class SyncReliableTxtStreamReader {
    ...
    readLine() {
        ...
        for (;;) {
            ...
            if (newlineIndex >= 0) {
                const lineBytes = current.subarray(0, newlineIndex);
                const lineStr = ReliableTxtDecoder.decodePart(lineBytes, this.encoding);
                this.rest = current.subarray(newlineIndex + 1);
                return lineStr;
            }
            else {
    ...
    getLine() {
        ...
        for (let i = length - 1; i >= 0; i--) {
            ...
            if (currentByte === 0x0A) {
                ...
                const lineBytes = this.buffer.subarray(sliceStart, sliceStart + sliceLength);
